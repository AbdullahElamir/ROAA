<?php
/*
Plugin Name: Ancora PayPal Donations
...
Text Domain: ancora-paypal-donations
...
spl_autoload_register('AncoraPayPalDonations::autoload');

/**
 * Init Singleton Class for Ancora PayPal Donations.
 *
 * @package Ancora PayPal Donations
 */
class AncoraPayPalDonations
{
    ...
    const OPTION_DB_KEY    = 'ancora_paypal_donations_options';
    const TEXT_DOMAIN      = 'ancora-paypal-donations';
    const FILE             = __FILE__;
    ...
    private function __construct()
    {
        ...
        $admin = new AncoraPayPalDonations_Admin();
        $admin->setOptions(
        ...
        add_shortcode('ancora-paypal-donation', array(&$this,'paypalShortcode'));
        add_action('wp_head', array($this, 'addCss'), 999);

        add_action(
            'widgets_init',
            create_function('', 'register_widget("AncoraPayPalDonations_Widget");')
        );
    }
    ...
    {
        delete_option('ancora_paypal_donations_options');
        delete_option('widget_ancora_paypal_donations');
    }
    ...
    public function addCss()
    {
        ...
        if (isset($opts['center_button']) and $opts['center_button'] == true) {
            ...
            echo '.ancora-paypal-donations { text-align: center !important }'."\n";
            echo '</style>'."\n";
        }
    }
    ...
    public function generateHtml(
        ...
        return AncoraPayPalDonations_View::render('paypal-button', $data);
    }
...
add_action('plugins_loaded', array('AncoraPayPalDonations', 'getInstance'));
