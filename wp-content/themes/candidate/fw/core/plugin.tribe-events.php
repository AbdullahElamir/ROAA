...
if (!function_exists('axiom_tribe_events_theme_setup')) {
    add_action( 'axiom_action_before_init_theme', 'axiom_tribe_events_theme_setup', 1 );
	function axiom_tribe_events_theme_setup() {
		if (axiom_exists_tribe_events()) {
            ...
            add_filter('axiom_filter_post_date',	 					'axiom_tribe_events_post_date', 9, 3);

            add_filter('axiom_filter_add_sort_order', 				'axiom_tribe_events_add_sort_order', 10, 3);
            ...
            add_filter('tribe_events_google_maps_api',     'axiom_tribe_events_google_maps_api');

            // Add query params to show events in the blog
            if (axiom_get_theme_option('show_tribe_events_in_blog')=='yes') {
                add_filter( 'posts_join',								'axiom_tribe_events_posts_join', 10, 2 );
                add_filter( 'getarchives_join',							'axiom_tribe_events_getarchives_join', 10, 2 );
               add_filter( 'posts_where',								'axiom_tribe_events_posts_where', 10, 2 );
                add_filter( 'getarchives_where',						'axiom_tribe_events_getarchives_where', 10, 2 );
                }

			// Extra column for events lists
			if (axiom_get_theme_option('show_overriden_posts')=='yes') {
                add_filter('manage_edit-'.Tribe__Events__Main::POSTTYPE.'_columns',			'axiom_post_add_options_column', 9);
                add_filter('manage_'.Tribe__Events__Main::POSTTYPE.'_posts_custom_column',	'axiom_post_fill_options_column', 9, 2);
			}
		}
	}
}
...
if ( !function_exists( 'axiom_is_tribe_events_page' ) ) {
	function axiom_is_tribe_events_page() {
        ...
        if (axiom_exists_tribe_events()) {
            ...
            $is = in_array($AXIOM_GLOBALS['page_template'], array('tribe-events/default-template'));
            if (!$is) {
            if (empty($AXIOM_GLOBALS['pre_query'])) {
            ...
            $is = !empty($AXIOM_GLOBALS['pre_query']->tribe_is_event)
            || !empty($AXIOM_GLOBALS['pre_query']->tribe_is_multi_posttype)
            || !empty($AXIOM_GLOBALS['pre_query']->tribe_is_event_category)
            || !empty($AXIOM_GLOBALS['pre_query']->tribe_is_event_venue)
            || !empty($AXIOM_GLOBALS['pre_query']->tribe_is_event_organizer)
            || !empty($AXIOM_GLOBALS['pre_query']->tribe_is_event_query)
            || !empty($AXIOM_GLOBALS['pre_query']->tribe_is_past);
            }
            }
            }
return $is;
...
// Add Google API key to the map's link
if ( !function_exists( 'axiom_tribe_events_google_maps_api' ) ) {
    //add_filter('tribe_events_google_maps_api', 'axiom_tribe_events_google_maps_api');
    function axiom_tribe_events_google_maps_api($url) {
        $api_key = axiom_get_theme_option('api_google');
        if ($api_key) {
            $url = axiom_add_to_url($url, array(
                'key' => $api_key
            ));
        }
        return $url;
...
if ( !function_exists( 'axiom_tribe_events_get_blog_type' ) ) {
	...
	function axiom_tribe_events_get_blog_type($page, $query=null) {
		...
		if (!is_search() && axiom_is_tribe_events_page()) {
...
if (!function_exists('axiom_tribe_events_add_sort_order')) {
    function axiom_tribe_events_add_sort_order($q, $orderby, $order) {
...
if ( !function_exists( 'axiom_tribe_events_posts_join' ) ) {
    // add_action( 'posts_join', 'axiom_tribe_events_posts_join', 10, 2 );
    function axiom_tribe_events_posts_join($join_sql, $query) {
...
if ( !function_exists( 'axiom_tribe_events_getarchives_join' ) ) {
    // add_action( 'getarchives_join', 'axiom_tribe_events_getarchives_join', 10, 2 );
    function axiom_tribe_events_getarchives_join($join_sql, $r) {
        global $wpdb;
        $join_sql .= " LEFT JOIN " . esc_sql($wpdb->postmeta) . " AS _tribe_events_meta ON " . esc_sql($wpdb->posts) . ".ID = _tribe_events_meta.post_id AND  _tribe_events_meta.meta_key = '_EventStartDate'";
        return $join_sql;
    }
...
if ( !function_exists( 'axiom_tribe_events_posts_where' ) ) {
    // add_action( 'posts_where', 'axiom_tribe_events_posts_where', 10, 2 );
    function axiom_tribe_events_posts_where($where_sql, $query) {
        if (!is_admin() && $query->is_main_query()) {
            if ($query->is_day || $query->is_month || $query->is_year) {
                global $wpdb;
                $where_sql .= " OR (1=1";
                ...
                $dt = $query->get('m');
                $y = $query->get('year');
                if (empty($y)) $y = (int) axiom_substr($dt, 0, 4);
                ...
                if ($query->is_month || $query->is_day) {
                    ...
                    if (empty($m)) $m = (int) axiom_substr($dt, 4, 2);
                    $where_sql .= " AND MONTH(_tribe_events_meta.meta_value)=".esc_sql($m);
                }
                if ($query->is_day) {
                    $d = $query->get('day');
                    if (empty($d)) $d = (int) axiom_substr($dt, 6, 2);
...
if ( !function_exists( 'axiom_tribe_events_getarchives_where' ) ) {
    // add_action( 'getarchives_where', 'axiom_tribe_events_getarchives_where', 10, 2 );
    function axiom_tribe_events_getarchives_where($where_sql, $r) {
        global $wpdb;
        // Posts type and date
...
if ( !function_exists( 'axiom_tribe_events_post_date' ) ) {
    //add_filter('axiom_filter_post_date', 'axiom_tribe_events_post_date', 9, 3);
    function axiom_tribe_events_post_date($post_date, $post_id, $post_type) {
