...
function axiom_shortcodes_show_field(field, tab_idx) {
	...
	for (var clone_num in field['params']) {
		...
		for (var param_num in field['params'][clone_num]) {
			...
			case 'hidden':
				output += '<input class="axiom_options_input axiom_options_input_hidden" name="' + id + '" id="' + id + '" type="hidden" value="' + param['value'] + '" data-param="' + param_num + '" />';
			break;
			...
			case "switch":
				...
				output += '<input name="' + id + '"'
					+ ' type="hidden"'
					+ ' value="' + (axiom_empty(param['value']) ? opt[0]['key'] : param['value']) + '"'
					+ ' data-param="' + param_num + '"'
					+ (!axiom_empty(param['action']) ? ' onchange="axiom_options_action_'+param['action']+'(this);return false;"' : '')
					+ ' />'
			...
			case 'select':
				...
				if (axiom_empty(param['style']) || param['style']=='select') {
					output += '<input class="axiom_options_input axiom_options_input_select" type="text" value="' + rez[1] + '"'
						+ ' readonly="readonly"'
						//+ (!axiom_empty(param['mask']) ? ' data-mask="'+param['mask']+'"' : '')
						+ ' />'
...
function axiom_shortcodes_action_button(data, type) {
	...
	var output = '<span class="' 
				...
				+ '"'
				+ (!axiom_empty(data['icon']) && !axiom_empty(data['title']) ? ' title="'+data['title']+'"' : '')
				...
				+ (!axiom_empty(data['captions']) && !axiom_empty(data['captions']['choose']) ? ' data-caption-choose="'+data['captions']['choose']+'"' : '')
				+ (!axiom_empty(data['captions']) && !axiom_empty(data['captions']['update']) ? ' data-caption-update="'+data['captions']['update']+'"' : '')
				+ '>'
