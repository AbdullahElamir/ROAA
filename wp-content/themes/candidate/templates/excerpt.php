			...
			<div class="post_content clearfix">
				...
				<?php
					...
					if (!axiom_sc_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
                        if (isset($post_options['stylebutton2']) &&  axiom_sc_param_is_on($post_options['stylebutton2'])){
                            ?><?php echo '<a class="sc_button_square sc_button_style_filled2 sc_button_size_small" href="'.esc_url($post_data['post_link']) .'"><span>'. esc_html($post_options['readmore']).'</span></a>';?>
                        <?php
                        }else{
