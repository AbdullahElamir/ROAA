...
global $AXIOM_GLOBALS;
            ...
            $above_footer_show  = axiom_get_custom_option('show_above_sidebar_footer');
            $footer_parts = explode(' ', $above_footer_show);
            $footer_tint  = !empty($footer_parts[0]) ? $footer_parts[0] : 'dark';
            $footer_style = !empty($footer_parts[1]) ? $footer_parts[1] : $footer_tint;
            if (!axiom_sc_param_is_off($above_footer_show)) {
                $AXIOM_GLOBALS['current_sidebar'] = 'footer';
                ?>
                <footer class="footer_wrap bg_tint_<?php echo esc_attr($footer_tint); ?> footer_style_<?php echo esc_attr($footer_style); ?> widget_area above-sidebar">
                    <div class="content_wrap">
                        <div class="columns_wrap">
                            <?php
                            do_action( 'before_sidebar' );
                            if ( is_active_sidebar( axiom_get_custom_option('sidebar_footer') ) ) { //remove it so SB can work
                                if ( ! dynamic_sidebar( axiom_get_custom_option('above_sidebar_footer') ) ) {
			...
			?> 
				<div class="copyright_wrap">
					<div class="content_wrap">
						<div class="copyright"><?php echo force_balance_tags(axiom_get_theme_option('footer_copyright')); ?>

                        </div>
                        <div class="copy_socials socPage">
                            <?php echo do_shortcode('[trx_socials]'); ?>
                        </div>
						<?php
						global $AXIOM_GLOBALS;
